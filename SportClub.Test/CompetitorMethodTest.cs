using Moq;
using SportClub.Application.Interface;
using SportClub.Domain.Entity;

namespace TestProject6;

public class CompetitorMethodTest
{
    [Fact]
    public void SaveTest()
    {
        var repo = new Mock<ICompetitorRepository>();
        var competitor = new Competitor()
        {
            GroupId = Guid.NewGuid(),
            IsPaid = true,
            MedicalExaminationExpiryDate = DateTime.Now,
            IdentityId = Guid.NewGuid(),
            Id = Guid.NewGuid()
        };
        var x = repo.Object;
        x.Save(competitor);
        
        repo.Verify(r => r.Save(competitor), Times.Once);
    }
    [Fact]
    public void UpdateTest()
    {
        var repo = new Mock<ICompetitorRepository>();
        var competitor = new Competitor()
        {
            GroupId = Guid.NewGuid(),
            IsPaid = true,
            MedicalExaminationExpiryDate = DateTime.Now,
            IdentityId = Guid.NewGuid(),
            Id = Guid.NewGuid()
        };

        var x = repo.Object;
        x.Update(competitor);
        
        repo.Verify(r => r.Update(competitor), Times.Once);
    }
    [Fact]
    public void GetAllTest()
    {
        var repo = new Mock<ICompetitorRepository>();
        var x = repo.Object;
        x.GetAll();
        
        repo.Verify(r => r.GetAll(), Times.Once);
    }
    [Fact]
    public void GetByIdTest()
    {
        var repo = new Mock<ICompetitorRepository>();
        var x = repo.Object;
        var guid = Guid.NewGuid();
        x.GetById(guid);
        
        repo.Verify(r => r.GetById(guid), Times.Once);
    }
    [Fact]
    public void GetAllWithNoGroupTest()
    {
        var repo = new Mock<ICompetitorRepository>();
        var x = repo.Object;
        x.GetAllWithNoGroup();
        
        repo.Verify(r => r.GetAllWithNoGroup(), Times.Once);
    }
    [Fact]
    public void DeleteTest()
    {
        var repo = new Mock<ICompetitorRepository>();
        var x = repo.Object;
        var guid = Guid.NewGuid();
        x.Delete(guid);
        
        repo.Verify(r => r.Delete(guid), Times.Once);
    }
    [Fact]
    public void GetPageableTest()
    {
        var repo = new Mock<ICompetitorRepository>();
        var x = repo.Object;
        x.GetPageable(1,5);
        
        repo.Verify(r => r.GetPageable(1,5), Times.Once);
    }
}